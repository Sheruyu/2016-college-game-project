...
public class DefaultVelocity : MonoBehaviour {
	...
	void FixedUpdate () {
		...
		if (target1.animationState) {
			rigidbody2D.velocity = new Vector2 (0, 0); //fruit stops during transition animations/pause
		}
		else if (target1.gameOver == false) {
			rigidbody2D.velocity = new Vector2 (xDirection, yDirection) * target1.speed;
